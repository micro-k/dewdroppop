...
update msg model =
    case msg of
        ...
        Solve ->
          let
            dummy=(List.minimum<|List.map (\d-> solveMinimum 1 d model (certain 0 model)|>Debug.log "candidate") model.dews)|>Debug.log "answers"
          in
            (model,Cmd.none)
...
mPhi model =
  if List.isEmpty model.splashes then
    model|>Debug.log "model"
  else
    mPhi (phi model)

solveMinimum:Int->Dew-> Model -> Int->List
solveMinimum number position model limit=
  let
    afterDrop=mPhi (drop position.x position.y model)
  in
    if List.isEmpty afterDrop.dews then
      number|>Debug.log "one of answers"
    else if number>=limit then
      100+limit|>Debug.log "cutoff"
    else
      let
        check=List.map (\d -> solveMinimum (number+1) d afterDrop limit) afterDrop.dews|>Debug.log "list"
      in
        case List.minimum check of
          Just num->
            num
          _->
            1000+model.mapSize|>Debug.log "error"
